...
const apiWeatherURL = 'https://api.weatherapi.com/v1/current.json?key=' + key + '&q=Tooele&aqi=no';
...
Bank.prototype.checkBalance = function() {
    console.log(
        `Branch/Account: ${this.branch}/${this.account} | ` + `balance: ${this.balance.toFixed(2)}`
    );
};

Bank.prototype.updateSum = function() {
    document.getElementById("balance-sum").innerHTML = `Account: ${demo.account}--------------------- $${demo.balance}`;
}
